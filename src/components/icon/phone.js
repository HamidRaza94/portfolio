import classNames from 'classnames';

import { VARIANT, PROP_TYPES, DEFAULT_PROPS } from './constants';

const Phone = (props) => {
  const { size, variant, animate } = props;

  if (variant === VARIANT.filled) {
    return (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width={size}
        height={size}
        viewBox="0 0 24 24"
        stroke-linecap="round"
        stroke-linejoin="round"
        className={classNames('stroke-1 stroke-black dark:stroke-white fill-black dark:fill-white')}
      >
        <path
          d="M9 3a1 1 0 0 1 .877 .519l.051 .11l2 5a1 1 0 0 1 -.313 1.16l-.1 .068l-1.674 1.004l.063 .103a10 10 0 0 0 3.132 3.132l.102 .062l1.005 -1.672a1 1 0 0 1 1.113 -.453l.115 .039l5 2a1 1 0 0 1 .622 .807l.007 .121v4c0 1.657 -1.343 3 -3.06 2.998c-8.579 -.521 -15.418 -7.36 -15.94 -15.998a3 3 0 0 1 2.824 -2.995l.176 -.005h4z"
          className={classNames(
            'stroke-black dark:stroke-white stroke-1 fill-black dark:fill-white',
            { 'group-hover:animate-vibrate': animate }
          )}
        />
        <path
          d="M15 7a2 2 0 0 1 2 2"
          className={classNames({ 'group-hover:animate-blink': animate })}
        />
        <path
          d="M15 3a6 6 0 0 1 6 6"
          className={classNames({ 'group-hover:animate-reverse-blink': animate })}
        />
      </svg>
    );
  }
};

Phone.propTypes = PROP_TYPES;
Phone.defaultProps = DEFAULT_PROPS;

export default Phone;
